...
class TestAIGeneratorToolCalling:
    ...
    def test_generate_no_tool_use(self, mock_anthropic_text_response):
        ...
        with patch('anthropic.Anthropic') as mock_anthropic_class:
            ...
            # Assert
            assert result == "MCP stands for Model Context Protocol. It's a standardized way for AI models to connect to data sources."
            mock_client.messages.create.assert_called_once()

    def test_generate_with_tool_use(self, mock_anthropic_tool_use_response, mock_anthropic_final_response, mock_tool_manager):
        ...
        with patch('anthropic.Anthropic') as mock_anthropic_class:
            ...
            # Assert
            assert result == "Based on the course materials, MCP (Model Context Protocol) is a protocol that allows AI models to connect to external data sources and tools."
    ...
    def test_handle_tool_execution_single(self, mock_anthropic_tool_use_response, mock_anthropic_final_response, mock_tool_manager):
        ...
        with patch('anthropic.Anthropic') as mock_anthropic_class:
            ...
            generator = AIGenerator(api_key="test_key", model="claude-sonnet-4-20250514")
            tool_definitions = [{"name": "search_course_content", "description": "Search"}]
    ...
    def test_handle_tool_execution_formats_messages(self, mock_anthropic_tool_use_response, mock_anthropic_final_response, mock_tool_manager):
    ...
    def test_tool_execution_error_propagation(self, mock_anthropic_tool_use_response, mock_anthropic_final_response, mock_tool_manager):
    ...
    def test_generate_with_tools_but_no_manager_raises_error(self, mock_anthropic_tool_use_response):
...
class TestSequentialToolCalling:
    ...
    def test_single_round_tool_use(self, mock_anthropic_tool_use_response, mock_anthropic_final_response, mock_tool_manager):
    ...
    def test_two_rounds_sequential_tools(self, mock_anthropic_multi_round_sequence, mock_tool_manager):
        """Complex query requiring two tool calls"""
        # Arrange
        with patch('anthropic.Anthropic') as mock_anthropic_class:
            mock_client = MagicMock()
            mock_client.messages.create.side_effect = mock_anthropic_multi_round_sequence
            mock_anthropic_class.return_value = mock_client
            ...
            # Assert
            assert result == "Based on the course outline and detailed search, MCP Architecture covers the core protocol design and implementation patterns."
            ...
            call_1_messages = mock_client.messages.create.call_args_list[0][1]['messages']
            call_2_messages = mock_client.messages.create.call_args_list[1][1]['messages']
            call_3_messages = mock_client.messages.create.call_args_list[2][1]['messages']

            assert len(call_1_messages) == 1  # Initial user query
            assert len(call_2_messages) == 3  # user, assistant(tool_use), user(tool_results)
            assert len(call_3_messages) == 5  # previous 3 + assistant(tool_use) + user(tool_results)

    def test_max_rounds_exceeded(self, mock_anthropic_infinite_tool_use, mock_tool_manager):
        ...
        with patch('anthropic.Anthropic') as mock_anthropic_class:
            ...
            assert mock_client.messages.create.call_count == 3  # Exactly MAX_TOOL_ROUNDS + 1 calls
            assert mock_tool_manager.execute_tool.call_count == 2  # Exactly MAX_TOOL_ROUNDS executions
    ...
    def test_tool_error_passed_to_claude(self, mock_anthropic_tool_use_response, mock_anthropic_final_response, mock_tool_manager):
        ...
        with patch('anthropic.Anthropic') as mock_anthropic_class:
            ...
            mock_tool_manager.execute_tool.return_value = "error: ChromaDB connection failed"
    ...
    def test_all_tools_fail_terminates_loop(self, mock_anthropic_tool_use_response, mock_tool_manager):
    ...
    def test_message_history_accumulation(self, mock_anthropic_multi_round_sequence, mock_tool_manager):
        ...
        with patch('anthropic.Anthropic') as mock_anthropic_class:
            ...
            round_1_messages = mock_client.messages.create.call_args_list[0][1]['messages']
            ...
            round_2_messages = mock_client.messages.create.call_args_list[1][1]['messages']
            ...
            final_messages = mock_client.messages.create.call_args_list[2][1]['messages']
            assert len(final_messages) == 5

    def test_backward_compatibility_single_tool(self, mock_anthropic_tool_use_response, mock_anthropic_final_response, mock_tool_manager):
